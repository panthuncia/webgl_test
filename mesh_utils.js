  ...
  for (let i = 0; i < indices.length; i += 3) {
    ...
    for (let mesh of subMeshes) {
      if (canMeshAccommodateJoints(mesh, jointSet, N)) {
        addTriangleToMesh(mesh, triangleIndices, triangleVertices, triangleNormals, triangleTexcoords, triangleBaryCoords, triangleTangents, triangleBitangents, triangleJoints, triangleWeights, mesh.indexMap);
        meshFound = true;
        break;
      }
    }
    ...
    if (!meshFound) {
      if (canMeshAccommodateJoints(currentMesh, jointSet, N)){
        addTriangleToMesh(currentMesh, triangleIndices, triangleVertices, triangleNormals, triangleTexcoords, triangleBaryCoords, triangleTangents, triangleBitangents, triangleJoints, triangleWeights, currentMesh.indexMap);
      }
      else {
...
function addTriangleToMesh(mesh, triangleIndices, triangleVertices, triangleNormals, triangleTexcoords, triangleBaryCoords, triangleTangents, triangleBitangents, triangleJoints, triangleWeights, indexMap) {
  ...
  triangleIndices.forEach((index, i) => {
    if (!indexMap.has(index)) {
      ...
      indexMap.set(index, currentIndex++);
    }
    mesh.indices.push(indexMap.get(index));
  });
  ...
  triangleJoints.forEach((joint) => {
    if (joint !== 0) {
      mesh.jointSet.add(joint);
      if (!mesh.jointMap.has(joint)) {
        mesh.jointMap.set(joint, mesh.jointMap.size);
        mesh.inverseJointMap.set(mesh.jointMap.size - 1, joint);
      }
    }
  });
}
...
function remapJointIndices(subMeshes) {
  subMeshes.forEach((mesh) => {
    ...
    for (let i = 0; i < mesh.joints.length; i += 4) {
      newJoints.push(mesh.jointMap.get(mesh.joints[i]), mesh.jointMap.get(mesh.joints[i + 1]), mesh.jointMap.get(mesh.joints[i + 2]), mesh.jointMap.get(mesh.joints[i + 3]));
    }
...
function createSubMeshes(gl, vertices, normals, texcoords, baryCoords, material, tangents, bitangents, indices, joints, weights, N) {
  ...
  let newMeshes = subMeshes.map((mesh) => new Mesh(gl, mesh.vertices, mesh.normals, mesh.texcoords, mesh.baryCoords, material, mesh.tangents, mesh.bitangents, mesh.indices, mesh.joints, mesh.weights, mesh.inverseJointMap));
