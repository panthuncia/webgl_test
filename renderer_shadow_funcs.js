WebGLRenderer.prototype.drawDepths = function (viewMatrix) {
  ...
  for (const key in this.currentScene.objects) {
    ...
    gl.uniformMatrix4fv(this.shadowScene.programInfo.uniformLocations.modelViewMatrix, false, modelViewMatrix);

    for (const mesh of object.meshes) {
      gl.bindBuffer(gl.ARRAY_BUFFER, mesh.vertexBuffer);
      gl.vertexAttribPointer(this.shadowScene.programInfo.uniformLocations.vertexPosition, 3, gl.FLOAT, false, 0, 0);
      gl.enableVertexAttribArray(this.shadowScene.programInfo.uniformLocations.vertexPosition);
  ...
  for (let key in this.currentScene.lights) {
    ...
    if (light.type == LightType.DIRECTIONAL) {
      ...
      for (let j = 0; j < this.NUM_SHADOW_CASCADES; j++) {
        ...
        gl.uniformMatrix4fv(this.shadowScene.programInfo.uniformLocations.projectionMatrix, false, cascadeInfo[j].orthoMatrix);
      ...
      directionalLightNum++;
        ...
        gl.uniformMatrix4fv(this.shadowScene.programInfo.uniformLocations.projectionMatrix, false, projectionMatrix);
