class WebGLRenderer {
  ...
  async drawScene() {
    ...
    for (const object of currentScene.objects) {
      ...
      gl.bindTexture(gl.TEXTURE_2D_ARRAY, currentScene.shadowScene.shadowMap); // Bind shadow map texture array
  ...
  async shadowPass() {
    ...
    for (let i = 0; i < this.NUM_SHADOW_CASCADES; i++) {
      gl.framebufferTextureLayer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, this.currentScene.shadowScene.shadowMap, 0, i);
  ...
  async initShadowScene() {
    const gl = this.gl;
    const numCascades = this.NUM_SHADOW_CASCADES;

    await this.createShadowProgram(); // Compile shaders

    this.currentScene.shadowScene.shadowFramebuffer = gl.createFramebuffer();
    gl.bindFramebuffer(gl.FRAMEBUFFER, this.currentScene.shadowScene.shadowFramebuffer);

    this.currentScene.shadowScene.shadowMap = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D_ARRAY, this.currentScene.shadowScene.shadowMap);

    // Use gl.DEPTH_COMPONENT32F for higher precision if needed
    gl.texImage3D(gl.TEXTURE_2D_ARRAY, 0, gl.DEPTH_COMPONENT24, this.SHADOW_WIDTH, this.SHADOW_HEIGHT, numCascades, 0, gl.DEPTH_COMPONENT, gl.UNSIGNED_INT, null);
    ...
    gl.framebufferTextureLayer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, this.currentScene.shadowScene.shadowMap, 0, 0);
  ...
  async createShadowProgram() {
    ...
    this.currentScene.shadowScene.programInfo = {
      ...
      uniformLocations: {
        ...
        cascade: gl.getUniformLocation(this.currentScene.shadowScene.shadowProgram, "u_cascade"),
      },
    };
  }
  createCallbacks() {
