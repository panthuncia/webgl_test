class WebGLRenderer {
    ...
    async drawScene() {
        ...
        for (const object of currentScene.objects) {
            ...
            let textureUnitAfterShadowMaps = 0;
            for (let i = 0; i < 1; i++) {
    ...
    async shadowPass() {
        ...
        let lightProjectionMatrix = this.currentScene.lights[0].projectionMatrix;
        let lightViewMatrix = this.currentScene.lights[0].viewMatrix;

        // Set uniform for the light's projection matrix
        gl.uniformMatrix4fv(this.currentScene.shadowScene.programInfo.uniformLocations.projectionMatrix, false, lightProjectionMatrix);
    ...
    async initShadowScene() {
        ...
        gl.bindTexture(gl.TEXTURE_2D, this.currentScene.shadowScene.shadowMap);

        // Use gl.DEPTH_COMPONENT32F for higher precision
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.DEPTH_COMPONENT24,
            this.SHADOW_WIDTH, this.SHADOW_HEIGHT, 0, gl.DEPTH_COMPONENT,
        ...
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT,
            gl.TEXTURE_2D, this.currentScene.shadowScene.shadowMap, 0);
